import { RelatedReflectionsSearch_GQL } from 'components/IntranetComponents/RelatedReflections/RelatedReflectionsSearch.graphql';
import type { QueryData } from 'components/IntranetComponents/RelatedReflections/RelatedReflections.types';
...
const COMPONENT = 'api:related-reflections';
const CACHE_TTL_SECONDS = 15 * 60;

/**
 * Returns reflections matching the supplied topic tag ids that the current
 * user is allowed to see. Filters by visibleBy server-side so unauthorized
 * reflections never reach the browser.
 */
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const cacheKey = `related-reflections:${siteHomeItemId}:${siteLanguage}:${sortedTagIds}`;
  ...
  if (allResults === null) {
    try {
      const tagPredicate = buildMultiFieldOrPredicateString(parsedTagIds, ['topicTags']);

      const graphQLClient = createGraphQLClientFactory({ api: scConfig.api })();

      let query = RelatedReflectionsSearch_GQL;
      query = query.replace('__TAG_PREDICATES__', tagPredicate);
      query = query.replace('__ANCESTOR_ID__', siteHomeItemId);
      query = query.replace('__TEMPLATE_ID__', TEMPLATE_ID_CONSTANTS.REFLECTION_DETAIL_PAGE);
      ...
      log('ERROR', COMPONENT, 'Failed to fetch related reflections', { error: String(error) });
      return res.status(500).json({ error: 'Failed to fetch related reflections' });
    }
  }
  ...
  log('INFO', COMPONENT, 'Filtered related reflections by visibleBy', {
