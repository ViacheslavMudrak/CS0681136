...
/** sessionStorage root for quotes tab filter state — keyed by account. */
export const ORDER_MANAGEMENT_QUOTES_FILTERS_STORAGE_KEY = "cp_order_management_quotes_filters_v1";
...
function isQuoteSortColumnId(x: unknown): x is SortColumnId {
  return (
    x === null ||
    x === "quoteId" ||
    x === "quoteContactPerson" ||
    x === "items" ||
    x === "quoteStatus" ||
    x === "quoteDate" ||
    x === "quoteExpiresIn" ||
...
function parsePersisted(raw: unknown): OrderManagementFiltersPersistedV1 | null {
  ...
  if (!isQuoteSortColumnId(o.sortColumn)) return null;
...
export function readOrderManagementQuotesFilters(
  accountId: string,
  ...
  try {
    const raw = sessionStorage.getItem(ORDER_MANAGEMENT_QUOTES_FILTERS_STORAGE_KEY);
    if (!raw) return null;
...
export function writeOrderManagementQuotesFilters(
  accountId: string,
  ...
  try {
    ...
    sessionStorage.setItem(ORDER_MANAGEMENT_QUOTES_FILTERS_STORAGE_KEY, JSON.stringify(root));
  } catch {
    // quota / private mode
  }
}
