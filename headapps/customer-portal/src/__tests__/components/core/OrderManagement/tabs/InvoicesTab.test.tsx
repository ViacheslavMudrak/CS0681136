...
vi.mock("@/components/core/OrderManagement/tabs/invoices/InvoicesSearchBarFilter", () => ({
  InvoicesSearchBarFilter: ({
    orderManagement,
  }: {
    orderManagement: { appliedSearch?: string; statusSelections?: Set<string> };
  }) => (
    <div data-testid="invoices-search-filter">
      {orderManagement.appliedSearch ?? "no-search"}-{orderManagement.statusSelections?.size ?? 0}
    </div>
  ),
}));
...
describe("InvoicesTab", () => {
  ...
  it("renders invoices tab stack when invoices tab is active", () => {
    renderWithToast(
    ...
    expect(screen.getByTestId("invoices-search-filter")).toBeInTheDocument();
    ...
    expect(screen.getByTestId("invoices-desktop-table")).toBeInTheDocument();
    expect(screen.getByTestId("invoices-mobile-cards")).toBeInTheDocument();
    expect(screen.getByTestId("order-management-pagination")).toBeInTheDocument();
  });

  it("passes applied search and status selection count to invoices filter", () => {
    mockUseOrderManagementShell.mockReturnValue(
      createShellMock({
        appliedSearch: "invoice-123",
        statusSelections: new Set(["open", "paid"]),
      })
    );
    ...
    expect(screen.getByText("invoice-123-2")).toBeInTheDocument();
  });

  it("does not render non-invoice tab controls for invoices tab kind", () => {
    renderWithToast(
      <OrderManagementDefaultVariant
        testId={TEST_CASE_DATA_IDS.ORDER_MANAGEMENT}
        fields={baseFields}
    ...
    expect(screen.queryByTestId("quotes-search-filter")).not.toBeInTheDocument();
